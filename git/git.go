...
// GuessRepoURL guesses the canonical URL for a repo, based on its
// configured remotes.
func GuessRepoURL(repoDir string) (*url.URL, error) {
...
func IndexGitRepo(opts build.Options, branchPrefix string, branches []string, submodules bool) error {
	...
	if url, err := GuessRepoURL(opts.RepoDir); err != nil {
