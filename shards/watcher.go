...
func newShardWatcher(dir string) *shardWatcher {
	return &shardWatcher{
		dir:      dir,
...
func (s *shardWatcher) scan() error {
	...
	loadThrottle := make(chan struct{}, runtime.NumCPU())
...
func (s *shardWatcher) loadShard(name string) {
	shard, err := loadShard(filepath.Join(s.dir, name))
	log.Printf("reloading: %s, err %v ", name, err)
	if err != nil {
		return
	}
	s.replace(name, shard)
}
