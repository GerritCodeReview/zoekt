...
func divertLogs(dir string, interval time.Duration) {
	...
	var last *os.File
	for {
		nm := fmt.Sprintf("zoekt-webserver.%d.%s.log", os.Getpid(), time.Now().Format(time.RFC3339Nano))
		nm = filepath.Join(dir, strings.Replace(nm, ":", "_", -1))
		...
		if err != nil {
			...
			fmt.Fprintf(os.Stderr, "can't create output file %s: %v", nm, err)
		}
