...
<script>
  ...
  function zoektAddQ(atom) {
      window.location.href = "/search?q=" + escape(zoektQuery + " " + atom) +
	  "&" + "num=" + zoektNum;
  }
</script>
<body id="results">
        ...
        <tr>
          <th>
            ...
            <small>
              ...
              {{if .Language}}<button onclick="zoektAddQ('lang:{{.Language}}'); return false;" class="label label-primary">{{.Language}}</button></span>{{end}}
