...
func runServer(logDir, indexDir string, port int, restart time.Duration) {
	for {
		nm := fmt.Sprintf("%s/webserver.%s.log", logDir, time.Now().Format(time.RFC3339Nano))
		...
		cmd := exec.Command("zoekt-webserver", "-listen", fmt.Sprintf(":%d", port))
		cmd.Stderr = f
		...
		timeOut := time.After(restart)
		select {
		case <-timeOut:
			// We kill the server once a day, to force logfiles to recycle.
			cmd.Process.Kill()
		case <-exitCh:
			// The server died
			log.Printf("unexpected webserver death: see %s", nm)
		}
	}
}
...
func main() {
	...
	restartInterval := flag.Duration("restart_interval", day, "restart the webserver this often")
	...
	runServer(logDir, indexDir, *port, *restartInterval)
}
