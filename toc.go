...
// MinReaderVersion and ReaderMinFeatureVersion constrain forwards and backwards
// compatiblity. For example, if a new way to encode filenameNgrams on disk is
...
// encoding and the associated reader can be removed, and MinReaderVersion and
// ReaderMinFeatureVersion can be set to the current FeatureVersion, indicating
...
// MinReaderVersion constrains forwards compatibility by emitting files
// that won't load in zoekt with a FeatureVersion below it.
const MinReaderVersion = 10

// ReaderMinFeatureVersion constrains backwards compatibility by refusing to
// load a file with a FeatureVersion below it.
const ReaderMinFeatureVersion = 8
