...
func TestIndexed(t *testing.T) {
	b := testIndexBuilder(t, nil,
		Document{Name: "f1", Content: []byte("foobar"), Indexed: true},
		Document{Name: "f3", Content: []byte("NOT-INDEXED: document larger than limit")},
		Document{Name: "f2", Content: []byte("NOT-INDEXED: probably not text"), Language: "Binary"})
	...
	if !matches[0].Indexed {
		t.Errorf("got %v, want indexed true", matches)
	}
	...
	if matches[0].Indexed || matches[1].Indexed {
		t.Errorf("got %v, want indexed false", matches)
	}
}
